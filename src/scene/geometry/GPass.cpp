...
namespace wntr {
	...
	std::shared_ptr<GPass> GPass::Create(const RenderContext* context) {
		return std::make_shared<GPass>(context);
    }
    
	GPass::GPass(const RenderContext* context)
    : RenderPass() {
		createProgram(context);
    }

	void GPass::drawBatch(RenderContext* context) {
		assert(context);
		...
		for (const auto& keys : _batch) {
			...
			if (keys.first) {
				...
				setTexture(TEXTURE_SPECULAR, keys.first->getTextureSpecular(), context);
				setTexture(TEXTURE_EMISSIVE, keys.first->getTextureEmissive(), context);
			}
			else {
	...
	void GPass::addToBatch(const std::shared_ptr<Entity>& entity, const glm::mat4& world, RenderContext* context) {
        assert(entity);
	...
	void GPass::createProgram(const RenderContext* context) {
		std::string vs_code =
			"#version 330\n"
			...
			"void main () {\n"
			"	vec4 world_position = world*vec4(attribute_position, 1.0f);\n"
			"	gl_Position = projection*view*world_position;\n"
			"	position = world_position.xyz;\n"
			"	normal = (world*vec4(attribute_normal, 0.0f)).xyz;\n"
			"   uv = attribute_uv;\n"
			"}\n";
